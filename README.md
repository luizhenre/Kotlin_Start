# 🚀 Kotlin Start — Estudos de Kotlin e Lógica de Programação

Este repositório contém meus estudos em Kotlin focados na base da programação e no desenvolvimento Android.

Aqui registro minha evolução prática em:
- Lógica de programação
- Kotlin
- Estruturas de controle
- Arrays
- Funções
- Organização de projetos
- Git e GitHub

---

## 🧠 Conteúdos já estudados

✔ Variáveis  
✔ Condicionais (`if`, `when`)  
✔ Laços (`for`, `while`, `do while`, `repeat`)  
✔ Arrays  
✔ Funções (parâmetros, retorno, reutilização)  
✔ Organização de código em múltiplos arquivos  

---

## 📦 Mini projetos

| Projeto                 | Descrição                                                                                   |
|-------------------------|---------------------------------------------------------------------------------------------|
| **Controle de Estoque** | Sistema em Kotlin via terminal para controle de produtos, relatórios e atualização de dados |
| **Controle de Alunos**  | Sistema em Kotlin via terminal para controle de notas, atualização e consulta de status     |

> Cada mini projeto possui seu próprio README dentro da pasta correspondente.

---